...
class StorageProvider:
    ...
    def add_depth_updates(self, depth_updates: pd.DataFrame):
        ...
        if len(self.depth_updates_df) >= self.batch_size:
            ...
            self.logger.info(
                f"chunk #{self.depth_updates_batches_saved}: saved {len(self.depth_updates_df)} depth updates"
            )
            self.depth_updates_df = pd.DataFrame()
    ...
    def add_trades(self, trades: pd.DataFrame):
        ...
        if len(self.trades_df) >= self.batch_size:
            ...
            self.logger.info(
                f"chunk #{self.trades_batches_saved}: saved {len(self.trades_df)} trades"
            )
            self.trades_df = pd.DataFrame()
...
class CsvStorage(StorageProvider):
    ...
    def __init__(self, market: str, symbol: str, batch_size: int, base_path: str):
    ...
    def _save_depth_updates(self):
        ...
        self.depth_updates_df.to_csv(
            self.depth_updates_path, index=False, mode="a", header=write_header
        )
    ...
    def _save_trades(self):
        ...
        self.trades_df.to_csv(
            self.trades_path, index=False, mode="a", header=write_header
        )
