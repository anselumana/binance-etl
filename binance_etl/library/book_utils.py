...
class OrderBookSynchronizer:
    ...
    def try_to_sync_book(self, update: dict):
        ...
        if self.initial_book_snapshot is None:
            ...
            try:
                self.initial_book_snapshot = self.binance_client.depth(
                    self.symbol.upper(), limit=1000
                )
                self.logger.info(
                    f"snapshot fetched (last update id: {self.initial_book_snapshot['lastUpdateId']})"
                )
            except Exception as ex:
                self.logger.warning(
                    f"failed to sync: unable to get initial snapshot from the REST API: {ex}"
                )
                return
        last_update_id = self.initial_book_snapshot["lastUpdateId"]
        valid_updates = [
            x for x in self.book_updates if x["last_update_id"] > last_update_id
        ]
        if len(valid_updates) == 0:
            self.logger.warning(
                f"failed to sync: all buffered deltas are older than the snapshot"
            )
            return
        ...
        for update in valid_updates:
            if (
                ...
                and update["last_update_id"] >= last_update_id + 1
            ):
        ...
        self.book_updates = [
            x
            for x in self.book_updates
            if x["first_update_id"] >= first_update_to_process["first_update_id"]
        ]
        self.logger.info(
            f"successfully synced book to first_update_id = {first_update_to_process['first_update_id']}"
        )
        self.is_synced = True
