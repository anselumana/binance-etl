...
class SpotDepthETL(BinanceETL):
    ...
    def _handle_message(self, message: dict):
        ...
        """
        self.logger.debug(
            f"processing update {message['last_update_id']} ({len(message['bids']) + len(message['asks'])} deltas)"
        )
        # ensure current update is consistent with last received
        if not self._is_consistent(message):
            raise Exception(
                "Unable to process update: it's not consistent with the last one received."
            )
        ...
        if not self.book_synchronizer.is_synced:
            ...
            if not self.book_synchronizer.is_synced:
                ...
                self._save_initial_snapshot(
                    self.book_synchronizer.initial_book_snapshot
                )
    ...
    def _deserialize_message(self, message: str) -> dict:
        ...
        try:
            ...
            self.logger.warning(
                f"Unable to deserialize depth update message: {ex}\nMessage body:\n{message}"
            )
        return res
    ...
    def _is_consistent(self, update: dict):
        ...
        if self.last_book_update is not None:
            ...
            if first_update_id != previous_delta_last_update_id + 1:
                self.logger.warning(
                    f"inconsistent update: current update has a diff of {first_update_id - previous_delta_last_update_id + 1} updates from the last ({first_update_id} vs {previous_delta_last_update_id})"
                )
                is_consistent = False
