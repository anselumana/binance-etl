...
class SpotTradesETL(BinanceETL):
    ...
    def _handle_message(self, message: str):
        ...
        """
        self.logger.debug(
            f"processing trade {message['id']} ({message['side']} {message['quantity']} @{message['price']})"
        )
        ...
        try:
            ...
            if "e" in trade.keys() and trade["e"] == "trade":
                res = {
                    ...
                    "quantity": trade["q"],
                    "side": (
                        "sell" if trade["m"] else "buy"
                    ),  # who is the liquidity aggressor (taker)
                }
        except Exception as ex:
            self.logger.warning(
                f"Unable to deserialize trade message: {ex}\nMessage body:\n{message}"
            )
        return res
